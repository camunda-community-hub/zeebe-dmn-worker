...
def jobName = 'zeebe-dmn-worker-DISTRO-maven-deploy'
def repository = 'zeebe-dmn-worker'
...
JAR="zeebe-dmn-worker-${RELEASE_VERSION}.jar"
...
./github-release release --user zeebe-io --repo zeebe-dmn-worker --tag ${RELEASE_VERSION} --name "Zeebe DMN Worker ${RELEASE_VERSION}" --description ""
./github-release upload --user zeebe-io --repo zeebe-dmn-worker --tag ${RELEASE_VERSION} --name "${JAR}" --file "${JAR}"
./github-release upload --user zeebe-io --repo zeebe-dmn-worker --tag ${RELEASE_VERSION} --name "${CHECKSUM}" --file "${CHECKSUM}"
'''
